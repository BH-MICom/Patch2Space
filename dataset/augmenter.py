...
def get_train_generator(trainloader):
    # set 2D and 3D augmentation parameters here
    dims = config.MODEL.NUM_DIMS
    # for random rotation
    angle_x = (-np.pi/6, np.pi/6) if dims == 3 else (-np.pi, np.pi)
    angle_y = (-np.pi/6, np.pi/6) if dims == 3 else (0., 0.)
    angle_z = (-np.pi/6, np.pi/6) if dims == 3 else (0., 0.)
    # for mirror
    mirror_axes = (0, 1, 2) if dims == 3 else (0, 1)
    ...
    scales = 2 / np.cumprod([2]*(len(config.MODEL.EXTRA.ENC_CHANNELS)-2))
