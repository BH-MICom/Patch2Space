...
def parse_args():
    ...
    parser.add_argument('--cfg', help='configuration file', type=str, default='configs/unet.yaml')
    ...
    parser.add_argument('--fold', help='which data fold to train on', type=int, default=0)
...
def main(args):
    ...
    for epoch in range(config.TRAIN.EPOCH):
        ...
        train(model, mri_class, ct_class, train_generator, optimizer, criterion, logger, config, epoch)
        scheduler.step()
        # running validation at every epoch is time consuming
        if epoch%config.VALIDATION_INTERVAL == 0:
            perf = inference(model, mri_class, ct_class, validset, logger, config)
